// importing
import {
  generateUserID, imgToBase64Converter, imgToBlobConverter, formValidator, initUI,
} from './src/utils.js';
...
function startGame() {
  ...
  function checkIfComplete() {
    if (cards.pairsNumber === turns.correct) {
      ...
      alert(`You have completed the game. Your score is: ${turns.score()} Would you like to go to ranking page?`);
    ...
    card.addEventListener('click', () => {
      if (cards.turnedNumber < 2) {
        ...
        if (cards.turnedNumber === 2
          ...
          setTimeout(() => {
            document.querySelectorAll('.game__card:not(.correct)').forEach((incorrectCard) => {
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (page === 'about' || page === 'ranking' || page === 'settings' || page === 'game' || page === 'reg') {
      if (page !== 'game') {
        ...
        gridSize = document.getElementById('difficulty') && (<HTMLInputElement>document.getElementById('difficulty')).value
          ? parseInt((<HTMLInputElement>document.getElementById('difficulty')).value, 10)
          : 4;
